...
out/avahi.hmod: src/avahi-0.7/avahi-daemon/.libs/avahi-daemon mod/bin/avahi-daemon mod/lib/ mod/lib/libexpat.so.1 mod/lib/libdaemon.so.0.5.0 mod/lib/libavahi-core.so.7.0.2 mod/lib/libavahi-common.so.3.5.3
	mkdir -p out/
	[ -f "out/avahi.hmod" ] || tar -czvf "out/avahi.hmod" -C mod/ etc usr bin lib
	
mod/lib/libexpat.so.1: mod/lib/
	[ -f "mod/lib/libexpat.so.1.6.0" ] || cp "src/expat-2.1.0/.libs/libexpat.so.1.6.0" "mod/lib/"
...
mod/lib/libdaemon.so.0.5.0: mod/lib/
	[ -f "mod/lib/libdaemon.so.0.5.0" ] || cp "src/libdaemon-0.14/libdaemon/.libs/libdaemon.so.0.5.0" "mod/lib/"
...
mod/lib/libavahi-core.so.7.0.2: mod/lib/
	[ -f "mod/lib/libavahi-core.so.7.0.2" ] || cp "src/avahi-0.7/avahi-core/.libs/libavahi-core.so.7.0.2" "mod/lib/"
...
mod/lib/libavahi-common.so.3.5.3: mod/lib/
	[ -f "mod/lib/libavahi-common.so.3.5.3" ] || cp "src/avahi-0.7/avahi-common/.libs/libavahi-common.so.3.5.3" "mod/lib/"
...
src/avahi-0.7/avahi-daemon/.libs/avahi-daemon: src/avahi-0.7/Makefile
	...
	[ -f "src/avahi-0.7/avahi-daemon/.libs/avahi-daemon" ] || make -C "src/avahi-0.7/"
...
src/avahi-0.7/Makefile: src/workbench/lib/libdaemon.la src/workbench/lib/libexpat.la src/avahi-0.7/configure
	...
	cd "src/avahi-0.7/"; \
	[ -f "Makefile" ] || ./configure --host arm-linux-gnueabihf --with-distro=none --prefix="`pwd`/../workbench/" --sysconfdir=/etc --localstatedir=/var --disable-glib --disable-qt3 --disable-qt4 --disable-gtk --disable-gtk3 --disable-gdbm --disable-pygtk --disable-mono --disable-monodoc --disable-stack-protector --disable-manpages --disable-autoipd --with-avahi-user=root --with-avahi-group=root --with-autoipd-user=root --with-autoipd-group=root --with-systemdsystemunitdir=no  --with-xml=expat --enable-compat-libdns_sd --disable-dbus --disable-glib --disable-gobject --disable-python --disable-python-dbus
	
src/avahi-0.7/configure: src/avahi-0.7.tar.gz
	tar -xzvf "src/avahi-0.7.tar.gz" -C "src/"

src/avahi-0.7.tar.gz:
	-mkdir -p src
	wget "https://github.com/lathiat/avahi/releases/download/v0.7/avahi-0.7.tar.gz" -O "src/avahi-0.7.tar.gz"
...
src/expat-2.1.0/configure: src/expat-2.1.0.tar.gz
	[ -f "src/expat-2.1.0/configure" ] || tar -xzvf src/expat-2.1.0.tar.gz -C "src/"

src/expat-2.1.0.tar.gz:
	mkdir -p src
	wget https://github.com/libexpat/libexpat/releases/download/R_2_1_0/expat-2.1.0.tar.gz -O "src/expat-2.1.0.tar.gz"
...
src/libdaemon-0.14/Makefile: src/libdaemon-0.14/Makefile.am
	...
	export ac_cv_func_setpgrp_void=1; \
...
src/libdaemon-0.14/Makefile.am: src/libdaemon-0.14.tar.gz
	[ -f "src/libdaemon-0.14/Makefile.am" ] || tar -xzvf "src/libdaemon-0.14.tar.gz" -C "src/"

src/libdaemon-0.14.tar.gz:
	mkdir -p src
	wget "http://0pointer.de/lennart/projects/libdaemon/libdaemon-0.14.tar.gz" -O "src/libdaemon-0.14.tar.gz"
